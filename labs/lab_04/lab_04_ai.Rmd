...
```{r}
library(axolotr)
create_credentials(GROQ_API_KEY = "NA")
```
...
film_tv_response <- axolotr::ask(
  prompt = paste("Given the following text, extract information on films and TV shows into a CSV file with the following structure, including headers, with no yapping: title, year, role, director, co-stars, and awards", text),
  model = "llama-3.1-8b-instant"
)
...
**Answer:** The information is not completely correct. It struggled with organzing the names of actors/actresses, and movie titles with names in them also tripped up Groq. It also appears that Groq pulled some information from the internet, as some of the names in the CSV aren't mentioned in the article.
...
umd_course_response <- axolotr::ask(
  prompt = paste("Given the following code, explain what exactly it does at what data it is extracting", r_code_example),
  model = "llama-3.1-8b-instant"
)
...
**Answer:** The answer does seem to be correct. It understands that the code is analyzing courses at UMD, seeking courses with "Climate" in the name that have at least one seat.
...
code_response <- axolotr::ask(
  prompt = paste("Given the following lab question and code, determine if the code can be improved in any way and if it properly answers the question provided", lab_question, your_r_code_lab),
  model = "llama-3.1-8b-instant"
)
...
**Answer: Groq did a solid job here. It identified that the code and the question are unrelated. It also provides new code that better answers the question.**
...
data_response <- axolotr::ask(
  prompt = paste("Given a dataset that provides general information about each traffic collision and details of all traffic collisions occurring on county and local roadways within Maryland's Montgomery County, as collected via the Automated Crash Reporting System (ACRS) of the Maryland State Police, provide five ideas for potential news stories that incorporate this data"),
  model = "llama-3.1-8b-instant"
)
...
**Answer: The story ideas are actually pretty good, in my opinion, and are ones that I would actually pursue, like which intersections and roadways are the most dangerous for pedestrians and other drivers.**
...
article_response <- axolotr::ask(
  prompt = paste("Using proper journalistic style, write a 500-word news article about a car crash at the intersection between Knox Road and Regents Drive"),
  model = "llama-3.1-8b-instant"
)
...
**Answer: The story does keep to the structure that one would expect from article like this, but the main problem of course is that nearly all of the details were filled in by Groq because very little was provided in the initial prompt. There are also some minor issues with the ordering of the paragraphs, but those are less dire than Groq's fabrications.**
