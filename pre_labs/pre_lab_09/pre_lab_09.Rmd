...
**Answer** Copy the url of the graphic you published here. It will begin with <https://datawrapper.dwcdn.net/>. Don't copy the browser's current url. <https://datawrapper.dwcdn.net/X5cD2/1/>
...
md_gov_county <- md_gov_county |>
  mutate(total=Cox + Moore + Lashar + Wallace + Harding,
         percentage_moore = (Moore/total*100))
...
**Task** Look for "Hide regions without data" under Appearance, and click the slider icon to enable that feature. You should see a map zoomed into Maryland with some counties in various colors. **Answer** Describe the changes from the previous map: **For some reason, I did not have another map. It took me straight to a zoomed in map of Maryland.**
...
**Task** Click the "Customize tooltips" button so it expands down. Change {{ fips_code }} to {{ county }} and {{ pctmoore }} to {{ FORMAT(pctmoore, "00.0%")}} **Answer** Describe the changes from the previous map: **The data is much easier to read with the county name and a simple percentage.**
...
<https://datawrapper.dwcdn.net/4mN6p/1/>
