...
```{r}
install.packages('tidyverse', 'rmarkdown')
library(tidyverse)
```
...
**Answer 79366 obs. and nine variables**
...
```{r}
department_count <- umd_courses |>
  group_by(term, department) |>
  summarise(
    count_classes = n()
  ) |>
  arrange(desc(count_classes), department)
```
...
1.  What department has the largest course sections on average?

2.  What course has the largest range of seats?

3.  Which instructor taught the most courses in a single term?
