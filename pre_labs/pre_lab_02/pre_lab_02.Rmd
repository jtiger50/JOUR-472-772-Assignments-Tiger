...
**Answer: Using a new variable is important because it keep the original journalism variable with all of the data intact. We can still go back and reference the entire journalism data set while also have selected information properly named and organized.**
...
**Answer: The expenditure date column now has the year moved to the beginning, and R is now able to properly read it as a date.**
...
**Answer: All of the values in the new month column are reading as the first of that month, which is not fully accurate to the data. My guess is that this is happening because R's \<date\> format needs to display a value for the day. I think it is defaulting to one rather than zero because it needs to be a real date.**
...
**Answer: This code is making the data easier to interpret on a larger scale by showing how many total votes there were and how many went to Wes Moore.**
...
**Answer: The Percentage is now easier to read and the counties are now supported by how many of its votes went to Moore. It shows that Prince George's County and Baltimore City supported Moore the most.**
...
**Answer: This method of sorting is interesting in its own way, but I would say its less useful overall. Since there are five total candidates, it is much more specific and valuable to know who voted for Wes Moore rather than who didn't.**
...
**Answer: I asked ChatGPT, "what are some tidyverse functions similar to str_to_upper"**

**It gave me str_to_lower, which changes to lowercase, and str_to_title, which changes to title case.**
...
**Answer: This answer does make sense to me. It is logical that Maryland and nearby areas like DC and Virginia would be at the top of the list. Then, it gets down to larger and more wealthy states like California, Florida and Massachusetts, which also makes sense.**
...
1.  In light of DEI in higher education becoming a target earlier this year, how many course titles include indentity-related words like "race" and "gender," making them potential targets?

2.  For the Maryland expenses, which payment methods were the most common and which brought in the most total money.

3.  How much money does Maryland spend on office supplies?
