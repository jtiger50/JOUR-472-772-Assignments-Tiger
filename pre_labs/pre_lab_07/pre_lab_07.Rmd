...
```{r echo=FALSE}
census_api_key("125d27a7e5e38fb0f7c621532fcde862e254d459", install=TRUE)
```
...
**Task** Run the following codeblock to find the Maryland county with the highest median household income and put the answer below. **Answer: Howard**
...
**Task** Run the following codeblock to create a chart showing the margin of error for each county. The current headline isn't very descriptive of the data. What should it be? **Answer: A better headline might be "Howard leads all Maryland counties in average household income"**
...
Is the difference between Calvert and Montgomery significant? **Answer: There is a difference, but not a significant one.**

Is the difference between Howard and everyone else significant? **Answer: Yes. Howard is by far the wealthiest even when accounting for margin of error.**
...
**Task** Let's look at the average household income for high schools that have 60% or more graduates immediately enrolling in college, and then look at those with less than 40%. Write a sentence describing the results. **Answer: There is a clear correlation between average income and college enrollment. Wealthier households are more likely to have children go to college.**

```{r}
md_high_schools_with_acs |>
  filter(percent_of_high_school_graduates_enrolled >= 60) |> 
  summarize(avg_income = median(estimate), count =n())
...
**Task** What is another question you could ask of this data? **Answer: Can this trend be observed ovr a longer period of time? As certain areas become more or less wealthy over the years, can we observe a clear correlation between college enrollment and it come as the latter rises and falls? Ex. If average income in Cecil County has declined relative to inflation over the last 10 years, is there also a similar decline in college enrollment?**
