...
**Task** Load some [Maryland state government payments data](https://opendata.maryland.gov/Budget/State-of-Maryland-Payments-Data-FY2008-to-FY2024/7syw-q4cy) by running the following code. We'll use the guess_max() function as an argument to use the first 10 rows to set the data type. What does the first line of the red Warning message that prints out when you load the data say? What happens when you follow its instructions? Answer below. **A: The first line is saying that there were issues in organizing the data, because the data likely had some input outliers that confused R. When I followed the instructions, the code went through without a warning.**

```{r}
payments <- read_csv("data/State_of_Maryland_Payments_Data__FY2008_to_FY2024_20250115.csv", guess_max=10)
dat <- read_csv("data/State_of_Maryland_Payments_Data__FY2008_to_FY2024_20250115.csv", guess_max=10)
problems(dat)
```

### Task 3: Reload data

**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer: There are no parsing errors with the higher guess_max.**
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer: Vendor name is \<chr\>. Amount is a \<dbl\>. Date is \<chr\>.**
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think explains those records? **Answer: There are 582 zip codes that are less than five characters. Most of these are likely errors when typing the zip code or international zip codes. There are some Canadian zip codes.**
...
**Task** Run the following codeblock and describe below what each line is doing. Looking at the results, what jumps out at you as potentially problematic? **Answer: There are multiple entries that aren't a loan or a contract. Some are N/A, one is just L and one says Contract.**
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer: There are many loans of only a few hunderd dollars or so with the smallest loan is for only \$60.**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does it change the headers? Answer below. **Answer: The columns are now all lowercase with underscores. This makes it easier for R to read them.**
...
**Task** Run the following codeblock to use str_to_upper() to make the source, grantee and description columns upper-cased. Describe what each line is doing. **Answer: The code is cleaning the names, renaming grantor to source and making the text in the data all uppercase.**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer: There are 58 rows, so there are 29 possible duplicates that can be deleted.**
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer: 19,453 compared to 19,482 in the old dataframe.**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer: A new column was added that restricts the value of zip codes to five characters at most, but the shorter values are still included.**
...
cleaned_md_grants_loans <- md_grants_loans |>
  ...
  mutate(source = str_to_upper(source), grantee = str_to_upper(grantee), description = str_to_upper(description)) |> 
  distinct() |>
  mutate(zip5 = str_sub(zip_code, start=7L, end=11L)) |>
